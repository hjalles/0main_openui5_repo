jQuery.sap.require("sap.ui.qunit.qunit-css");
jQuery.sap.require("sap.ui.thirdparty.qunit");
jQuery.sap.require("sap.ui.qunit.qunit-junit");
QUnit.config.autostart = false;

// We cannot provide stable mock data out of the template.
// If you introduce mock data, by adding .json files in your webapp/localService/mockdata folder you have to provide the following minimum data:
// * At least 3 Suppliers in the list
// * All 3 Suppliers have at least one Products

sap.ui.require([
	"sap/ui/test/Opa5",
	"com/prego/test/mobileHybridCRUD/test/integration/pages/Common",
	"sap/ui/test/opaQunit",
	"com/prego/test/mobileHybridCRUD/test/integration/pages/App",
	"com/prego/test/mobileHybridCRUD/test/integration/pages/Browser",
	"com/prego/test/mobileHybridCRUD/test/integration/pages/Master",
	"com/prego/test/mobileHybridCRUD/test/integration/pages/Detail",
	"com/prego/test/mobileHybridCRUD/test/integration/pages/Create",
	"com/prego/test/mobileHybridCRUD/test/integration/pages/NotFound"
], function (Opa5, Common) {
	"use strict";
	Opa5.extendConfig({
		arrangements: new Common(),
		viewNamespace: "com.prego.test.mobileHybridCRUD.view."
	});

	sap.ui.require([
		"com/prego/test/mobileHybridCRUD/test/integration/MasterJourney",
		"com/prego/test/mobileHybridCRUD/test/integration/NavigationJourney",
		"com/prego/test/mobileHybridCRUD/test/integration/NotFoundJourney",
		"com/prego/test/mobileHybridCRUD/test/integration/BusyJourney",
		"com/prego/test/mobileHybridCRUD/test/integration/FLPIntegrationJourney"
	], function () {
		QUnit.start();
	});
});